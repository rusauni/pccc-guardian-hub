...
const ProfessionalSkillsPage = () => {
  ...
  const [professionalGuidelines, setProfessionalGuidelines] = useState<NewsItem[]>([]);
  ...
  const fetchNews = async () => {
    try {
      ...
      // Use the slug from URL or fallback to 'huong-dan-nghiep-vu'
      const categorySlug = slug || 'huong-dan-nghiep-vu';
      ...
      const mappedNews = posts.map(post => ({
        ...
        category: {
          name: post.category?.name || 'Hướng dẫn nghiệp vụ',
          slug: post.category?.slug || categorySlug,
          id: post.category?.id
        }
      }));
      
      setProfessionalGuidelines(mappedNews);
    } catch (err) {
  ...
  if (isLoading && professionalGuidelines.length === 0) {
    return (
  ...
  if (error && professionalGuidelines.length === 0) {
    return (
      <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
        <Navbar />
        <Breadcrumbs />
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <main className="flex-grow bg-white dark:bg-gray-900 py-8">
        <div className="container mx-auto px-4">
          ...
          {isLoading && professionalGuidelines.length > 0 && (
            <div className="flex justify-center py-8">
              <Loader2 className="h-8 w-8 animate-spin text-pccc-primary" />
            </div>
          )}
          
          {error && professionalGuidelines.length > 0 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {professionalGuidelines.map((item) => (
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="inline-block bg-gray-100 dark:bg-gray-700 text-xs px-2 py-1 rounded-full">
                      {item.category?.name || 'Hướng dẫn nghiệp vụ'}
                    </span>
                    <a 
          ...
          {professionalGuidelines.length === 0 && !isLoading && !error && (
