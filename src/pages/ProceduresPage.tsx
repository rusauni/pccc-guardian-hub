...
const ProceduresPage = () => {
  ...
  const [procedures, setProcedures] = useState<NewsItem[]>([]);
  ...
  const fetchNews = async () => {
    try {
      ...
      // Use the slug from URL or fallback to 'thu-tuc-hanh-chinh'
      const categorySlug = slug || 'thu-tuc-hanh-chinh';
      ...
      const mappedNews = posts.map(post => ({
        ...
        category: {
          name: post.category?.name || 'Thủ tục hành chính',
          slug: post.category?.slug || categorySlug,
          id: post.category?.id
        }
      }));
      
      setProcedures(mappedNews);
    } catch (err) {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <main className="flex-grow bg-white dark:bg-gray-900 py-8">
        <div className="container mx-auto px-4">
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="inline-block bg-gray-100 dark:bg-gray-700 text-xs px-2 py-1 rounded-full">
                      {item.category?.name || 'Thủ tục hành chính'}
                    </span>
                    <a 
          ...
          {procedures.length === 0 && !isLoading && !error && (
